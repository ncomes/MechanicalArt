...
def zero_flags_cmd():
    ...
    if not selection:
        maya_dialogs.info_prompt(title='Nothing Selected', text='Please select a FRAG rig.')
        return
...
def select_all_flags_cmd():
    selection = pm.selected()
    if not selection:
        maya_dialogs.info_prompt(title='Select All', text='Please make sure you select at least one FRAG rig!')
