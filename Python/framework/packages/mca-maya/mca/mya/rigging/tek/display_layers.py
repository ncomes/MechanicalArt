...
class DisplayLayers(tek_base.TEKNode):
	VERSION = 1
	...
	def create(tek_parent):
		...
		:param TEKNode tek_parent: parent tek node.
		:return: Returns the display layers tek node.
		:rtype: TEKNode
		"""
		
		display_children = tek_parent.get_tek_children(of_type=DisplayLayers)
		if display_children:
		...
		node = tek_base.TEKNode.create(tek_parent, DisplayLayers.__name__, DisplayLayers.VERSION)
