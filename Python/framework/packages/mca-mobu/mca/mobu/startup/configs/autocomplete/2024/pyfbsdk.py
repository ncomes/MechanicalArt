...
def FBGetCharacterComparisonThresholdValue():
    """
...
def FBSetCharacterComparisonThresholdValue(pValue):
    """
...
class FBVideoGrabOptions (object):
    ...
    StillImageCompression=property(doc="<b>Read Write Property:</b>         ")
    StillImageQuality=property(doc="<b>Read Write Property:</b> Quality of the image for image formats that support it. Depends on the image format. For 'jpeg': 0-100 where 0=(low visual quality, high compression), 100=(high visual quality, low compression).         ")
    StillImageCompressionType=property(doc="<b>Read Write Property:</b> Compression type of image for image formats that support it. Depends on the image format. For 'tif': 0 = ( no compression ), 1 = ( LZW-compression ).         ")
...
class FBApplicationState (Enumeration):
    """
    Application state the MotionBuilder application has been started in.     
         
    """
    kFBInteractive=property(doc="Running in an interactive mode with the UI. Default mode.          ")
    kFBBatch=property(doc="Running in a batch mode, without the UI, i.e.: when starting MotionBuilder from the command line with the -batch command flag.          ")
    kFBMobuPy=property(doc="Running via the mobupy executable.          ")
    pass
...
class FBDeckTransportMode (Enumeration):
    ...
    kFBDeckTransportSlave=property(doc="        ")
    kFBDeckTransportMain=property(doc="Transport main.          ")
    kFBDeckTransportMaster=property(doc="        ")
    pass
...
class FBFileCreation (Enumeration):
    """
    Types of operation to execute when trying to write a file that already exists on disk.     
         
    """
    kFBOverwrite=property(doc="Overwrite the existing file.          ")
    kFBMerge=property(doc="Merge with the existing file. Not applicable when exporting with the FBX file format.          ")
    kFBCancel=property(doc="Cancel the export of the current motion file only. Execution will continue for the other motion files, if any.          ")
    kFBCancelAll=property(doc="Cancel the export of the current motion file and of all the remaining motion files, if any.          ")
    pass
...
class FBModelSelection (Enumeration):
    ...
    kFBCreateModels=property(doc="Import context: Will create the models in the motion file, used when there is no model to match in the scene. Export context: Not applicable.          ")
    kFBSelectedModels=property(doc="Import context: Will merge data with only the selected nodes or models. Export context: Will export data from only the selected models in your scene.          ")
    kFBSelectedModelAndChildren=property(doc="Import context: Will try to match the models from the file to those selected in the scene, as well as the children of the selected models. Export context: Will export data from only the selected models in your scene.          ")
    kFBPrefixGroupContainingModel=property(doc="Import context: Will find the top node with the same prefix and imports the motion as if you selected kFBInHierarchy. If the selected node has the prefix, this merge option is the same as selecting kFBSelectedModelAndChildren. If no nodes are found with the prefix, this merge option operates the same as kFBSelectedModels. Only applicable when one model is selected. Export context: Will find the top node in the hierarchy with the same prefix and export all nodes in the hierarchy. Only applicable when one model is selected.          ")
    kFBInHierarchy=property(doc="Import context: Will find the root node and will try to merge the data on the hierarchy. Only applicable when one model is selected. Export context: Will find the root node and export the entire hierarchy. Only applicable when one model is selected.          ")
    kFBAllModels=property(doc="Import context: Will import motion into the hierarchies of all models in your scene. This is the only merge option when nothing is selected. Export context: Will treat the selected node as a root node and export its data and the data of its children. Only applicable when one model is selected.          ")
    pass
...
class FBPropertyFlag (Enumeration):
    ...
    kFBSlaveSetByMaster=property(doc="        ")
...
class FBActionManager (FBComponent):
    ...
    """
    def RescanCurrentInteractionModeShortcuts(self):
        """
    ...
    def RescanPythonShortcuts(self):
        """
...
class FBAnimationLayer (FBComponent):
    ...
    Color=property(doc="<b>Read Write Property:</b> The animation layer color.          ")
...
class FBApplication (FBComponent):
    ...
    def FileExportWithOptions(self,pOptions):
        """
        ...
        return : True if the export succeeded. Returns false if some files were not written (file not overwritten, invalid path, etc.). Returns false if the given options object is not valid. See the FBMotionFileExportOptions::IsValid method. 
        """
        pass
    ...
    def FileOpen(self,p0,pBufferLength):
        ...
        p0 : the memory buffer for the file. Raw memory address is expected in Python. 
    ...
    ApplicationState=property(doc="<b>Read Only Property:</b> State the MotionBuilder application has been started in.          ")
...
class FBControlSet (FBComponent):
    ...
    def GetFKIndex(self,pModel):
        ...
        pModel : FK Model to obtain index. 
        return : The index of the given FK model in the internal array or -1 if the model is not an FK model. 
        """
        pass
    ...
    def GetFKModel(self,pIndex):
        ...
        pIndex : Index associated to the FK model. 
        return : The FK model at the specified index or nullptr (None in Python) if no model is found. 
        """
        pass
    ...
    def GetFKName(self,pIndex):
        ...
        pIndex : Index to obtain name. 
        return : The name of FK model or nullptr (Empty string in Python) if the index is invalid. 
        """
        pass
    ...
    def GetIKEffectorIndex(self,pModel):
        ...
        pModel : IK Effector model to obtain index. 
        return : The index of the given IK Effector model (matches the FBEffectorId enum values) or -1 if the model is not an IK Effector. 
        """
        pass
    ...
    def GetIKEffectorModel(self,pEffectorIndex,pPivotIndex):
        ...
        pEffectorIndex : Index of the IK Effector. 
        pPivotIndex : Index of the IK Effector pivot. 
        return : The IK Effector model at the specified indices or nullptr (None in Python) if no model is found. 
        """
        pass
    ...
    def GetIKEffectorName(self,pEffectorIndex):
        ...
        pEffectorIndex : Index of the IK Effector. 
        return : The name of the IK Effector or nullptr (Empty string in Python) if the index is invalid. 
        """
        pass
    ...
    def GetIKEffectorPivotCount(self,pEffectorIndex):
        ...
        pEffectorIndex : Index of the IK Effector. 
        return : The number of IK Effector pivot or 0 if the index is invalid. 
        """
        pass
...
class FBFCurve (FBComponent):
    ...
    def KeysAdd(self,pTimes,pValues,pInterpolations,pTangentModes):
        """
    ...
    def KeysSetValues(self,pIndices,pValues,pInterpolations,pTangentModes):
        """
...
class FBImage (FBComponent):
    ...
    def WriteToDisk(self,pFileName,pComments,pQuality,pCompressionType):
        """
...
class FBModuleManager (FBComponent):
    ...
    def GetModuleModFilePath(self,pModuleName):
        """
        Returns the *.mod file path containing the definition of the provided loaded module name.

        pModuleName : The loaded module name to query. 
        return : The *.mod file path. 
        """
        pass

    def GetModulePath(self,pModuleName):
        """
    ...
    def GetModuleVersion(self,pModuleName):
        """
...
@endcode
    ...
    """
    def AddEdit(self,pName,pSetAsCurrent):
        """
        Add a new Edit in the Motion Blend.

        pName : The new Edit name. If unset, the name "Edit" will be set. 
        pSetAsCurrent : True (default) to set the new Edit as the current Edit to be used in the Motion Blend. 
    ...
    def GetEdit(self,pIndex):
        """
        Get the Edit associated to the input index.

        pIndex : The index of the Edit to get. 
    ...
    def GetSyncTakeEditStartEnd(self):
        """
    ...
    def RemoveEdit(self,pEdit):
        """
        Remove an Edit from the Motion Blend.

        pEdit : The FBMotionBlendEdit object to remove. 
        return : True if the operation is successful, false otherwise. 
        """
        pass

    def SetCurrentEdit(self,pEdit):
        """
        Set the current Edit to be used in the Motion Blend.

        pEdit : The FBMotionBlendEdit object to set as the current Edit. 
        return : True if the operation is successful, false otherwise. 
        """
        pass

    def SetForceTime(self,pForceTime):
        """
        Set the 'Force Time' state.

        pForceTime : True to snap the Timeline indicator in place so that it cannot be moved by dragging, false otherwise. 
        return : True if the operation is successful, false otherwise. 
        """
        pass

    def SetSnapOnFrame(self,pSnap):
        """
        Set the 'Snap On Frame' state.

        pSnap : True to force the start and end times of all cuts and poses to snap to the nearest frame, false otherwise. 
        return : True if the operation is successful, false otherwise. 
        """
        pass

    def SetSyncTakeEditStartEnd(self,pSync):
        """
        Set the 'Sync Take/Edit Start & End' state.

        pSync : True to sync the Take start/end times with the Take start/end times stored in the Motion Blend Edit selected, false otherwise. 
        return : True if the operation is successful, false otherwise. 
        """
        pass
...
Motion Blend Edit      
    """
    def FBMotionBlendEdit(self,pName,pSetAsCurrent):
        """
        Constructor.
    ...
    def SetKeepActive(self,pKeepActive):
        """
        Set the 'Keep Edit Active' state.

        pKeepActive : True to allow the result of the Edit to be shown while working on other Edits, false otherwise. 
        return : True if the operation is successful, false otherwise. 
        """
        pass
    ...
    def SetShowAllGhosts(self,pShow):
        """
        Set the 'Show All Ghosts' state.

        pShow : True to display all of the Edit ghosts in the Viewer window, false to hide them all. 
        return : True if the operation is successful, false otherwise. 
        """
        pass
...
class FBMotionFileExportOptions (FBComponent):
    ...
    """
    def FBMotionFileExportOptions(self,pFilePath):
        """
        Constructor.
    ...
    def GetFilePath(self):
        ...
        return : The file path for the motion file(s) to create. 
        """
        pass
    ...
    def GetTakeCount(self):
        ...
        return : The take count that can be exported. 
        """
        pass

    def GetTakeFilePath(self,pTakeIndex):
        """
        Get the export file path for given take.

        pTakeIndex : Index of take to get the file path. 
        return : The file path of the given take. Returns nullptr (None in Python) if the operation cannot be performed. 
        """
        pass

    def GetTakeFrameRate(self,pTakeIndex):
        """
        ...
        return : The frame rate of the given take. 
        """
        pass
    ...
    def GetTakeName(self,pTakeIndex):
        ...
        return : The name of the given take. Returns nullptr (None in Python) if the operation cannot be performed. 
        """
        pass
    ...
    def GetTakeSelect(self,pTakeIndex):
        ...
        return : True is the take will be exported, false otherwise. 
        """
        pass
    ...
    def GetTakeStart(self,pTakeIndex):
        ...
        return : The start time of the given take. 
        """
        pass
    ...
    def GetTakeStop(self,pTakeIndex):
        ...
        return : The stop time of the given take. 
        """
        pass

    def GetTakeTimeMode(self,pTakeIndex):
        """
        Get the time mode of the given take.

        pTakeIndex : Index of take to get the time mode. 
        return : The time mode of the given take. 
        """
        pass
    ...
    def IsValid(self):
        ...
        return : True if the provided file path for the motion file(s) to create is valid and if the take info is up to date with the scene, false otherwise. 
        """
        pass

    def ResetTakeInfo(self):
        """
    ...
    def SetFilePath(self,pFilePath):
        """
        ...
        pFilePath : The file path for the motion file(s) to create. 
        return : True if the provided file path is valid, false otherwise. 
        """
        pass

    def SetTakeFilePath(self,pTakeIndex,pFilePath):
        """
    ...
    def SetTakeTimeMode(self,pTakeIndex,pTimeMode,pCustomFrameRate):
        """
    ...
    AddPrefix=property(doc="<b>Read Write Property:</b> Effective only if the OneTakePerFile property is true. True to prefix the constructor's specified filename in the take export file path, false otherwise. Changing this property value will reset any user-specified file path set with the SetTakeFilePath method.          ")
    FileCreation=property(doc="<b>Read Write Property:</b> Operation to execute when trying to write a file that already exists on disk.          ")
    ModelSelection=property(doc="<b>Read Write Property:</b> Specifies the content to export.          ")
    OneTakePerFile=property(doc="<b>Read Write Property:</b> True to export each take in a separated file, false to export all takes in a single file. Changing this property value will reset any user-specified file path set with the SetTakeFilePath method.          ")
    pass
...
class FBMotionFileImportOptions (FBComponent):
    ...
    """
    def FBMotionFileImportOptions(self,pStringList):
        """
        Constructor.
...
class FBReferenceTime (FBComponent):
    ...
    UseRelativeLocalTime=property(doc="<b>Read Write Property:</b> True to show the relative local time, false otherwise. This will only be effective when displaying the Local Time, but the state can be changed even if not currently displaying the Local Time.          ")
    pass
...
class FBStory (FBComponent):
    ...
    def PlotAllShotTracksToCameraSwitcher(self,pTake):
        """
        ...
        pTake : Take to plot the Shot Tracks to the Camera Switcher to. 
        return : True if the operation is successful, false otherwise. 
        """
        pass

    ClipsTextsVisible=property(doc="<b>Read Write Property:</b> If true, clips' texts are visible.          ")
    DisplayShotClipsExpanded=property(doc="<b>Read Write Property:</b> If true, shot clips are displayed in expanded mode (each Shot track reflected as a new line in the Shot clips pane), otherwise, are displayed in collapsed mode (all Shot tracks reflected as a single line in the Shot clips pane).          ")
...
class FBStoryClip (FBComponent):
    ...
    def GetAssignSourcesToDestinationsInfo(self,pSrcList,pValidAnimatedSrc,pAvailableDstList,pValidAnimatedDst,pDefaultDstList,pEffectiveDstList):
        """
        GetAssignSourcesToDestinationsInfo.
    ...
    def GetSourceTimeFromDestinationTime(self,pDestinationTime,pLoopNumber):
        """
    ...
    ShowEmbeddedTimecode=property(doc="<b>Read Write Property:</b> Whether to show the embedded timecode (real, if available, or custom) of the video.          ")
...
See samples: MergePreviewAnimationLayers.py, ExportAnimationLibrary.py, GoToNextTake.py, GoToPreviousTake.py, MirrorPoseOverTime.py, MultiLayerKeying.py, RenameFirstTakeOnMultipleFiles.py, SaveOneTakePerFile.py, TimeCodeKeying.py.     
    ...
    def PlotTakeOnProperties(self,pPlotOptions,pPropertiesToPlot):
        """
...
class FBTimeWarpManager (FBComponent):
    ...
    def RemoveTimeWarpFromScene(self,pTake,pTimeWarp):
        """
...
@endcode
    ...
    def SetPreventDocking(self,pPreventDocking):
        """
        Set the 'Prevent Docking' state value.
        Note: When 'Prevent Docking' state is true, it is possible to temporary allow docking a window by holding the SHIFT key before moving it. When 'Prevent Docking' state is false, it is possible to temporary prevent docking a window by holding the CTRL key before moving it.

        pPreventDocking : The 'Prevent Docking' state value. 
        return : True if the operation is successful, false otherwise. 
        """
        pass
...
See samples: StartDevice.py, StopDevice.py.     
    ...
    def DeviceSendCommand(self,pOperation):
        """
...
class FBImageContainer (FBVisualComponent):
    ...
    UseTransparentBackground=property(doc="<b>Read Write Property:</b> True to specify that the image is using a transparent background, false otherwise.          ")
    pass
...
class FBSpread (FBVisualComponent):
    ...
    See samples: ActionScriptMgr.py, Spread.py.     
    """
    def FBSpread(self):
        """
        Constructor.
    ...
    def SetCellValue(self,pRow,pColumn,pValue):
        ...
        pRow : Row of cell. 
        pColumn : Column of cell. 
        pValue : Value of the cell (can be str (single or tuple), int, float or FBTime) 
        """
        pass
...
class FBSpreadRow (FBSpreadPart):
    ...
    RowVisible=property(doc="<b>Read Write Property:</b> Is row visible? Set to true to show the row, false to hide the row.          ")
    pass
...
class FBCamera (FBModel):
    ...
    ViewShowManipulators=property(doc="<b>Read Write Property:</b> Show manipulators?          ")
...
class FBConstraintRelation (FBConstraint):
    ...
    ConstraintRelation class.     
     This class exposes the relation constraint and allows addition of new boxes and removal of existing ones. See samples: CreateAndPopulateAConstraintRelation.py, TraversingRelationConstraint.py.     
    """
...
class FBStoryTrack (FBConstraint):
    ...
    def CopyTakeIntoTrack(self,pTimeSpan,pTake,pConnectToCurrentTake,pOutputOffset,pMakeUndoable):
        """
        CopyTakeIntoTrack Copy animation from the specified take for affected objects of the track.
    ...
    def PlotToCameraSwitcher(self,pTake):
        """
        PlotToCameraSwitcher Plot the Shot Track to the Camera Switcher on the given Take.
        This undoable operation will overwrite the Camera Switcher of Take.

        pTake : Take to plot the Shot Track to the Camera Switcher to. 
        return : True if the operation is successful, false otherwise. 
        """
        pass
...
class FBVideoClip (FBVideo):
    ...
    CustomEmbeddedStartTimecode=property(doc="<b>Read Write Property:</b>The custom timecode of the first frame of the video. To be effective, the ShowEmbeddedTimecode and UseCustomEmbeddedStartTimecode properties must be active.          ")
    ...
    RelativePath=property(doc="<b>Read Only Property:</b> Relative path of media.          ")
    ShowEmbeddedTimecode=property(doc="<b>Read Write Property:</b> True if the video clip is showing its embedded timecode (real, if available, or custom), false if showing a timecode starting at 00:00:00:00 (00).          ")
    ...
    UseCustomEmbeddedStartTimecode=property(doc="<b>Read Write Property:</b> True if the video clip is using the custom embedded start timecode value, false if it is using the real embedded timecode, if available.          ")
...
class FBVideoClipImage (FBVideoClip):
    """
    Video clip image media class.     
    Used for a variety of image types. For DDS images, use the FBVideoClipImageDDS instead.      
    """
...
class FBVideoClipImageDDS (FBVideoClipImage):
    ...
    """
    def FBVideoClipImageDDS(self,pName):
        """
        Constructor.

        pName : Name of DDS image file. 
        """
        pass
