...
# ==============================================================================
# Utilities for query mya node info
# ==============================================================================
...
def _getMayaTypes(real=True, abstract=True, basePluginTypes=True, addAncestors=True,
    ...
    basePluginTypes : bool
        Include the set of "base" plugin mya types (these are not returned by
    ...
    if abstract or addAncestors:
        ...
        for node in raw:
            if node.endswith(_ABSTRACT_SUFFIX):
                ...
                # For some reason, mya returns these names with cmds.allNodeTypes(includeAbstract=True):
...
def getInheritance(mayaType, checkManip3D=True, checkCache=True,
    ...
    # which means we don't need a real node. However, in mya < 2012, nodeType
...
class ApiMelBridgeCache(BaseApiClassInfoCache):
    ...
    def applyComments(cls, origText, origTextNoComments, newPath):
        ...
        def git(arg, output=False):
            ...
            # mya, it's unable to access the user's .gitconfig for some reason
...
class ApiCache(BaseApiClassInfoCache):
    ...
    # very least, inappropriate for determining which mya node types are
    # compatible with which MFn classes.
    # As an example, MFnContainerNode().type() will return MFn.kContainerBase;
    # however, kContainerBase clearly corresponds to the containerBase mya
    ...
    def _buildApiTypesList(self):
        ...
        mya type, it will be associated with
    ...
    def _buildMayaNodeInfo(self):
        '''Stores tempory information about mya nodes + names
        '''
    ...
    def _buildApiTypeToApiClasses(self):
        ...
        # all of mya OpenMaya api is now imported in module api's namespace
        ...
        # we do this by querying the mya hierarchy, and marching up it until
    ...
    def _buildApiRelationships(self):
        ...
        WARNING: will load all mya-installed plugins, without making an
        ...
        crash mya, especially if done from a non-GUI session
        """
        ...
        # mya.standalone.initialize()
        # import mya.cmds
        # mya.cmds.loadPlugin('C:\\3D\\Autodesk\\Maya2012\\bin\\plug-ins\\Mayatomr.mll')
        ...
        # before building the cache.  This is because some "core" mya api types
    ...
    def filterPluginNodes(self):
        ...
        - they map to an MFn enum other than one of the standard kPlugin*
          types - ie, some "core" mya types are actually implemented in
          plugins
        ...
        for mayaType, apiType in self.mayaTypesToApiTypes.items():
            if not apiType.startswith('kPlugin') or apiType == 'kPlugin':
                continue
            # don't remove the mya types for the kPlugin* themselves - ie,
