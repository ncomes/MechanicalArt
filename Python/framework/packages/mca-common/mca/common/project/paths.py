...
from mca.common.utils import dcc_util, list_utils, path_utils
...
def get_asset_maya_rig_file_path(asset_id):
    ...
    rig_path = list_utils.get_first_in_list([x for x in rig_list if x.lower().endswith('_rig.ma')])
    return rig_path or ''
...
def get_unreal_content_path():
    ...
    _path = os.path.join(project_paths.MCA_UNREAL_ROOT, 'unreal', 'Game', 'Content')
    return os.path.normpath(_path)
