...
def parseVersionStr(versionStr, extension=False):
    ...
    if 'Preview' in versionStr:
        ...
        # parse both the crazy human readable mya versions as returned by
        # about, and the mya location directory.  to handle both of these i'm
        # afraid the regular expression might be getting unwieldy

        ma = re.search(r"((?:mya)?(?P<base>[\d.]{3,})(?:(?:[ ].*[ ])|(?:-))?(?P<ext>x[\d.]+)?)", versionStr)
...
def flavor():
    ...
    try:
        ...
        raise RuntimeError("This method cannot be used until mya is fully initialized")
...
def isEval():
    ...
    try:
        ...
        raise RuntimeError("This method cannot be used until mya is fully "
                           "initialized")
