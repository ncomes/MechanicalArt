...
class PinComponent(keyable_component.KeyableComponent):
    ...
    def create(tek_parent,
        ...
        pin_flag = tek_flag.Flag(fk_result['flags'][0])
        ...
        node.connect_nodes(pin_flag, 'flags', 'tekParent')
        node.connect_nodes(pin_joint, 'pinJoint', 'tekParent')
        node.connect_nodes(bind_joint, 'bindJoints')
        node.connect_nodes(pin_node, 'pinNode', 'tekParent')
