...
These do not require initialization of mya.standalone"""
...
def getMayaLocation(version=None):
    ...
    # get the path of a different mya version than current
    if version:
...
def getMayaAppDir(versioned=False):
    ...
    if appDir is None:
        ...
        if platform.system() == 'Darwin':
            appDir = os.path.join(appDir, 'Library/Preferences/Autodesk/mya')
        else:
            appDir = os.path.join(appDir, 'mya')
...
def executeDeferred(func, *args, **kwargs):
    ...
    """
    This is a wrap for mya.utils.executeDeferred.  Maya's version does not
    ...
    `mya.utils.executeDeferred`, and if we're in batch mode, it just executes
    the function.

    Use this function in your userSetup.py file if:
        1. you are importing pymel there
        2. you want to execute some code that relies on mya.cmds
