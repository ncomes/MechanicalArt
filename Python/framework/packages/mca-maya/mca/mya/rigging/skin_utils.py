...
from mca.mya.utils import dag_utils, naming, attr_utils, fbx_utils
...
def get_influences(skinned_mesh, skip_namespace=False):
    ...
    skin_cluster_name = get_skincluster(skinned_mesh)
...
def get_skincluster(skin_obj):
    ...
    skin_cluster = list_utils.get_first_in_list([pm.PyNode(mobject) for mobject in skin_cluster_mobj_list])
...
def copy_skin_weights(source_list, target_list, match_influences, max_influences=DEFAULT_MAX_INFLUENCES):
            ...
            for y in [x] + pm.listRelatives(x, ad=True, type=pm.nt.Transform):
                if (isinstance(y, pm.nt.Transform) and y.getShape() and y not in filtered_list)\
                    ...
                    if not index:
                        ...
                        skin_cluster_node = get_skincluster(y[0].node()) if isinstance(y, list)\
                                               and isinstance(y[0], pm.MeshVertex) else get_skincluster(y)
    ...
    for target_node in filtered_target_list:
        ...
        skin_cluster_node = get_skincluster(base_node)
        if not skin_cluster_node:
            try:
    ...
    for node in search_list:
        if node and node.getShape():
            ...
            if skin_cluster:
                ...
                if influence_list:
                    bind_root = dag_utils.get_absolute_parent(influence_list[0], pm.nt.Joint)
...
def swap_vertex_influence_weight_cmd():
    ...
    if vertex_operators:
        mesh_skin_cluster = get_skincluster(pm.PyNode(vertex_operators[0]).node().getParent())
    else:
...
def set_skin_weights(weight_dictionary, blend_weights, shape_node, influence_set=None, dual_quat=False):
    ...
    skin_cluster = get_skincluster(shape_node)
    if skin_cluster:
        skin_cluster.unbind()

    attr_utils.set_attr_state(shape_node, False)
...
def get_skin_weights(mesh_node):
    ...
    skin_cluster = get_skincluster(mesh_node)
...
def set_vertex_weights(vert_ids, mesh_name, weight_map):
    ...
    skin_cluster = get_skincluster(pm.PyNode(mesh_name))
    if not skin_cluster:
        return False
...
def auto_skin(archetype, main_mesh):
    ...
    # Check for bind root
    root_joint = joint_utils.get_all_hierarchy_bind_roots(main_mesh)
    if not root_joint:
        if pm.objExists('root'):
            ...
            skel_path = assetlist.get_asset_skeleton(archetype)
...
def get_mirrored_weights(mesh, vertex_list):
    ...
    source_skin_cluster = get_skincluster(pymel_vert.node().getParent())
    vertices = {}
...
def mirror_vertex_weights_cmd(skip_dialog=False):
    ...
    if not vertices_list:
        return
    source_skin_cluster = get_skincluster(vertices_list[0].node().getParent())
    cluster_joints = pm.skinCluster(source_skin_cluster, q=True, wi=True)
    root_joint = dag_utils.get_absolute_parent(cluster_joints[0], pm.nt.Joint)
    children_joints = dag_utils.get_ordered_hierarchy(root_joint, pm.nt.Joint)
    main_joint = None
...
def copy_skin_weights(source_list, target_list, match_influences, max_influences=DEFAULT_MAX_INFLUENCES):
    ...
    filtered_target_list = []
    match_skin_cluster_list = []
    for index, (original_list, filtered_list) in enumerate([(source_list, filtered_source_list), (target_list, filtered_target_list)]):
        for x in original_list:
            if isinstance(x, pm.nt.Transform):
                # For the whole transform hierarchy
                for y in [x] + pm.listRelatives(x, ad=True, type=pm.nt.Transform):
                    if isinstance(y, pm.nt.Transform) and y.getShape() and y not in filtered_list:
                        # for each shaped transform
                        if not index:
                            # if we're running through the source list
                            skin_cluster_node = get_skincluster(y)
                            if not skin_cluster_node:
                                # node can't be a source if it has no skinCluster
                                continue
                            match_skin_cluster_list.append(skin_cluster_node)
                        filtered_list.append(y)
            elif isinstance(x, pm.MeshVertex):
                # For each mesh vertex
                skin_cluster_node = x.node()
                match_skin_cluster_list.append(skin_cluster_node)
...
def rebind_meshes_to_skel(node_list, bind_root):
    """
    ...
    for node in node_list:
        cluster = get_skincluster(node)
        if cluster:
            node_cluster_list.append([node, cluster])
            original_influence_list.update(set(cluster.getInfluence()))

    original_bind_names = [naming.get_basename(x) if not naming.get_basename(x).startswith('root') else 'root' for x in
                           original_influence_list]
    target_skel_dict = joint_utils.get_skel_dict(bind_root)
    match_influence_list = []
    for joint_name in original_bind_names:
        target_influence = target_skel_dict.get(joint_name, None)
        if not target_influence:
            logger.warning(f'Failed to find {joint_name} on the target skeleton, skin copy may not be ideal.')
    ...
    for node, cluster in node_cluster_list:
        ...
        pm.skinCluster(dupe, match_influence_list, tsb=True,
        ...
        pm.skinCluster(node, match_influence_list, tsb=True,
                                           normalizeWeights=1, maximumInfluences=4,
                                           obeyMaxInfluences=True, removeUnusedInfluence=False)
        pm.select(dupe, node)
        pm.copySkinWeights(noMirror=True, sa='closestPoint', ia=['oneToOne', 'name', 'closestJoint'], normalize=True)
...
def bind_and_copy_from_file(node_list, root_joint, file_path, only_unbound=False):
    """
    From a given file import skinned meshes then use them as the source for copying weights onto a given skeleton.

    :param list[Transform] node_list:
    :param Joint root_joint: The root joint of the skeleton we want to bind to. (All joints under the root will be valid influences.)
    :param str file_path: The absolute path to a file for importing.
    :param bool only_unbound: If only unbound meshes should have their weights copied.
    """
    if not node_list:
        logger.error('No meshes found to bind.')
        return

    if not root_joint:
        logger.error('No skeleton found to bind to.')
        return

    unbound_list = []
    for node in node_list:
        if not get_skincluster(node):
            unbound_list.append(node)

    operand_list = node_list
    if only_unbound:
        operand_list = unbound_list

    if not operand_list:
        return

    influence_list = pm.listRelatives(root_joint, ad=True, type=pm.nt.Joint)
    imported_node_list = []
    ...
    skinned_meshes = None
    if imported_node_list:
        skinned_meshes = [x for x in pm.ls(imported_node_list, type=pm.nt.Transform) if get_skincluster(x)]

    for node in operand_list:
        pm.skinCluster(node, influence_list, tsb=True,
                       normalizeWeights=1, maximumInfluences=4,
                       obeyMaxInfluences=True, removeUnusedInfluence=False)
        if skinned_meshes:
            # Just in case the file is a dud and is empty.
            pm.select(skinned_meshes, node)
            pm.copySkinWeights(noMirror=True, sa='closestPoint', ia=['oneToOne', 'name', 'closestJoint'], normalize=True)
...
def create_cluster_from_component_names(points, name, relative=False, front_of_chain=True, exclusive=False):
    ...
    :return: The newly created cluster and the handle object for it.
    :rtype: list(Cluster, Handle)
    """
...
def get_skeleton_meshes(root_joint):
    """
    Return a list of all bound meshes to this skeletal hierarchy.

    :param Joint root_joint: Root joint of a skeletal hierarchy
    :return: A list of all the shaped transforms influenced by this skeletal hierarchy.
    :rtype: list(Transform, ...)
    """
    node_list = set()
    cluster_list = set()
    for joint_node in [root_joint]+pm.listRelatives(root_joint, ad=True, type=pm.nt.Joint):
        joint_clusters = set(joint_node.listConnections(type=pm.nt.SkinCluster))
        for cluster_node in joint_clusters:
            if cluster_node in cluster_list:
                continue
            cluster_list.add(cluster_node)
            node_list.update(set([n.getParent() for n in pm.skinCluster(cluster_node, q=True, g=True)]))
