...
methods described above) and which to not.  for instance, in my case, it is desirable for all of mya's global
...
There is no direct equivalent in python to the catch and catchQuiet command and it does not exist in mya.cmds so i wrote two
...
This variety of for loop has a direct syntactical equivalent in python.  the only catch here is that mya.cmds
...
def mel2py(input, outputDir=None,
    ...
    input
        ...
        which relies on the script already being sourced by mya.
