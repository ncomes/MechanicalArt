...
class SpaceSwitchComponent(frag_base.FRAGNode):
	VERSION = 1
	...
	def create(frag_parent,
		...
		node.connect_node(switch_offset_grp, "switchGroup", 'fragParent')
		...
		node.connect_nodes(point_constraints, 'pointConstraints', 'fragParent')
		node.connect_nodes(orient_constraints, 'orientConstraints', 'fragParent')
		node.connect_nodes(conditions, 'conditions', 'fragParent')
