...
class AnimatedCurvesComponent(tek_base.TEKNode):
	VERSION = 1
	...
	def create(tek_parent):
		...
		:param tek_rig.FragRig tek_parent: The Frag Node that this node will be connected to.
		...
		node = tek_base.TEKNode.create(tek_parent,
											tek_type=AnimatedCurvesComponent.__name__,
	...
	def store_curves_on_skeleton(self, skeleton_root, keep_animation=True):
		...
		control_rig = tek_base.get_tek_parent(self)
		if not control_rig:
		...
		if all_flags:
			...
			pm.disconnectAttr(self.pynode.tekParent)
	...
	def store_animation_curves(self,
		...
		obj_name = naming.get_basename(obj)
		component = tek_base.get_tek_parent(obj)
	...
	def export(self, full_path):
		...
		if ns:
			...
			pm.rename(self, new_name)
		if self.pynode.tekParent.listConnections():
			pm.disconnectAttr(self.pynode.tekParent)
