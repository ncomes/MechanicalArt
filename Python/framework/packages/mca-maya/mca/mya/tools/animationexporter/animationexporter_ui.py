...
from mca.common.utils import path_utils, string_utils
...
class AnimationExporter(mayawindows.MCAMayaWindow):
    ...
    def initialize_tabs(self):
        ...
        frag_sequencer.reconnect_orphaned_sequences()
        ...
        for frag_root in frag_root_list:
            ...
            rig_tab_index = self.ui.tabWidget.addTab(rig_tab_widget, f'{frag_root.pynode.namespace().replace(":", "")}:{frag_root.asset_name}')
        ...
        sequences_dict = frag_sequencer.get_sequences()
        for rig_pynode, sequence_wrapper_dict, in sequences_dict.items():
            if not rig_pynode:
                continue
            frag_rig = frag.FRAGNode(rig_pynode)
    ...
    def _toggle_local_export(self):
        ...
        rig_pynode = self._rig_list[index]

        self.toggle_export([rig_pynode], self._rig_dict[rig_pynode]['toggle_rig_export_checkbox'])
    ...
    def add_sequence_cmd(self):
        ...
        """
        index = self.ui.tabWidget.currentIndex()
        frag_rig = frag.FRAGNode(self._rig_list[index])
    ...
    def add_sequence(self, frag_rig):
        ...
        """
        if not pm.objExists(frag_rig.pynode):
            self.initialize_tabs()
    ...
    def export_sequences(self, all_rigs=False):
        ...
        ae_utils.export_frag_sequences_cmd(self._rig_list, sequences_to_skip=sequences_to_skip)
...
class SequenceFrameButton(qtwidgets.QFrame):
...
class AnimationExporterSequence(common_windows.ParentableWidget):
    ...
    def _update_path(self):
        ...
        mca_asset = assetlist.get_asset_by_id(self.frag_rig.asset_id)
        animation_path = None
        if mca_asset:
            animation_path = path_utils.to_relative_path(mca_asset.animation_path)
    ...
    def remove_sequence(self):
        ...
        frag_sequencer = frag.get_frag_sequencer()
        frag_sequencer.remove_sequence(sequence_path, self.frag_rig)
    ...
    def set_sequence(self, found_sequence_path=None):
        ...
        """
        if not pm.objExists(self.frag_rig.pynode):
        ...
        if self.sequence_path != found_sequence_path:
            frag_sequencer.remove_sequence(self.sequence_path, self.frag_rig)
            SEQUENCE_PATH_DICT.pop(self.sequence_path, None)

        frame_range = self._get_frame_range()
        frag_sequencer.set_sequence(found_sequence_path, self.frag_rig, frame_range=frame_range,
    ...
    def export_sequence(self):
        ...
        if not pm.objExists(self.frag_rig.pynode):
            ...
            messages.info_message('Export Failure', 'One or more frag rigs were unable to be located in the scene. Please ensure that the rig exists in the scene.', icon='error')
            return
        ae_utils.export_frag_sequences_cmd([self.frag_rig], sequences_to_skip=[x for x in sequence_dict[self.frag_rig] if x != self.sequence_path])
    ...
    def move_sequence(self, positive_move=True):
        ...
        """
        frag_sequencer = frag.get_frag_sequencer()
        if frag_sequencer.reorder_sequence(self.sequence_path, self.frag_rig, positive_move):
