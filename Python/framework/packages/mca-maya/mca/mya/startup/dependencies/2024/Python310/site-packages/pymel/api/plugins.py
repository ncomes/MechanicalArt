...
An example of a plugin which creates a node::
    ...
    class PymelSineNode(plugins.DependNode):
        '''Example node adapted from mya's example sine node plugin
        ...
        # builtin library must be functional / working from within mya... due
        ...
        # override and set your own mya node name, do this:
        #_name = 'PymelSineNode'
...
# Because different versions of mya may not have all these MPxNodes, we need
...
# Gives a map from an MPx class name to it's mya node type name
...
# it's possible that multiple mpxNames map to the same mya name  -
...
# Harcode these, and error if any other mya type has multiple
# mpx names
...
for mpxName, mayaName in mpxNamesToMayaNodes.items():
    ...
    if existingMpxName is not None:
        msg = "Encountered unexpected mapping of multiple MPx nodes ({}, {})" \
              " to the same mya node ({})".format(mpxName, existingMpxName,
                                                   mayaName)
...
def _suggestNewMPxValues(classes=None):
    ...
    import pymel.core  # need mya.cmds
    import pprint
...
# We want to make sure we know if mya adds a new MPx class!
...
class BasePluginMixin(object):
    ...
    def register(cls, plugin=None):
        """Used to register this MPx object wrapper with the mya plugin.
...
def _buildPluginHierarchy(dummyClasses=None):
    ...
    This command must be run from within a running mya session - ie, where
    mya.cmds, etc are accessible.
    '''
...
#    try:
#        global registered
#        commands = mya.cmds.pluginInfo(_pluginName(), query=1, command=1)
...
def filterPlugins(plugins, filters):
    ...
    for plug in plugins:
        if not any(test(plug) for test in filterTests):
            ...
            logger.debug("Filtering mya plugin: {}".format(plug))
...
def mayaPlugins(filters=(), loaded=None):
    ...
    '''All mya plugins in the mya install directory
    ...
    # always include some plugins in the list of mya plugins, even though
    # they're installed in a different dir
    plugins = ['mtoa']
    pluginPath = os.environ.get('MAYA_PLUG_IN_PATH')
    if pluginPath is None:
        raise RuntimeError("mya.standalone.initialize() must be called before"
                           " this function")
...
def loadAllMayaPlugins(filters=()):
    ...
    '''will load all mya-installed plugins
    ...
    WARNING: tthe act of loading all the plugins may crash mya, especially if
    ...
    while unloadedPlugins and loadedAPlugin:
        passNum += 1
        logger.debug("loading all mya plugins (pass {})...".format(passNum))
        loadedAPlugin = False
        ...
        for plugin in thisPass:
            try:
                ...
                loadedAPlugin = True
    logger.debug("...done loading all mya plugins")
...
def unloadAllPlugins(skipErrors=False, exclude=('DirectConnect',)):
    ...
    if loadedPlugins:
        ...
        # mya.cmds.unloadPlugin(force=True, *loadedPlugins)
...
UNREPORTED_COMMANDS = {
    'command': {},       # all versions of mya should support this flag!
...
def pluginCommands(pluginName, reportedOnly=False):
    ...
    Note that depending on your version of mya, this may not actually be the
