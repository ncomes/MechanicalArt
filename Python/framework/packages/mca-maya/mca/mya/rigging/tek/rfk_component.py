...
class RFKComponent(keyable_component.KeyableComponent):
    ...
    def create(tek_parent,
        ...
        node.connect_nodes(mid_flags, 'midFlags', 'tekParent')
        node.connect_nodes(dup_chain, 'rfkChain', 'tekParent')
        node.connect_node(end_twist_flag, 'topFlag', 'tekParent')
        node.connect_node(start_twist_flag, 'bottomFlag', 'tekParent')
        node.connect_nodes(between_joints, 'bindJoints')
        node.connect_nodes(mid_groups, 'midGroups', 'tekParent')
        node.connect_nodes(mid_offset_groups, 'midOffsetGroups', 'tekParent')
        node.connect_nodes(reverse_nodes, 'reverseNodes', 'tekParent')
        if sub_flags:
            node.connect_nodes(sub_flags, 'subFlags', 'tekParent')
    ...
    @property
    def start_flag(self):
        return tek_flag.Flag(self.bottomFlag.get())

    @property
    def end_flag(self):
        return tek_flag.Flag(self.topFlag.get())
