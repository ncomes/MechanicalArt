"""
Contains the wrapping mechanisms that allows pymel to integrate the api and mya.cmds into a unified interface
"""
...
class MelCommandMissingError(MissingInCacheError):
    ...
    def str(self):
        return "mel command {} cannot be found - perhaps it is not available" \
               " in this version of mya?".format(self.cmdName)
...
class ApiMethodMissingError(MissingInCacheError):
    ...
    def str(self):
        return "method {} of {} cannot be found - perhaps it is not available" \
               " in this version of mya?".format(self.methodName,
                                                  self.apiClassName)
...
# Dictionary mapping from mya node type names (ie, surfaceShape) to pymel
...
def getCmdFunc(cmdName):
    ...
    if func is None:
        def nonExistantFunc(*args, **kwargs):
            ...
            This should generally only happen if using an "old" mya version
            that doesn't have a command found in newer mya versions
            '''
...
def fixCallbacks(inFunc, commandFlags, funcName=None):
    """
    Prior to mya 2011, when a user provides a custom callback functions for a
    ...
    def register(cls, apiTypeName, pymelType, inCast=None, outCast=None, apiArrayItemType=None):
        ...
        if apiTypeName in ['float', 'double', 'bool', 'int', 'short', 'long', 'uint']:
            ...
            for i in [2, 3, 4]:
                ...
                # Check if there is an explicit mya type for n of these - ie,
                #   int2 myVar;
...
class ApiUndo(with_metaclass(util.Singleton, object)):
    ...
    awhile back.  it works like this:
        ...
        - the API queue is a list of simple python classes with undoIt and
          ...
          the garbage node's integer attribute using mya.cmds.
        - when mya's undo or redo is called, it triggers the undoing or
          redoing of the garbage node's attribute change (bc we changed it using
          MEL/mya.cmds), which in turn triggers our API callback.  the callback
...
def wrapApiMethod(apiClass, methodName, newName=None, proxy=True, overloadIndex=None):
    ...
    piggybacks mya's MEL undo system, it won't get cross-mojonated.
...
class _MetaMayaCommandWrapper(MetaMayaTypeWrapper):
    ...
    """
    A metaclass for creating classes based on a mya command.
...
class MetaMayaNodeWrapper(_MetaMayaCommandWrapper):
    ...
    def __new__(cls, classname, bases, classdict):
        ...
        # mapping from pymel type to mya type should always be made...
        ...
        if oldMayaType is None:
            ...
            _logger.raiseLog(_logger.WARNING,
                             'creating new pymel node class %r for mya node '
                             'type %r, but a pymel class with the same name '
                             'already existed for mya node type %r' % (
                                 classname, nodeType, oldMayaType))

        # mapping from mya type to pymel type only happens if it's NOT a
        # virtual class...
        if not isVirtual:
            ...
            if oldPymelType is None:
                ...
                _logger.raiseLog(_logger.WARNING,
                                 ...
                                 'class %r for the same mya node type' % (
...
class MetaMayaUIWrapper(_MetaMayaCommandWrapper):
    ...
    """
    A metaclass for creating classes based on on a mya UI type/command.
    """
...
def addCustomPyNode(module, mayaType, extraAttrs=None):
    ...
    """
    create a PyNode, also adding each member in the given mya node's
...
def _addPyNode(module, mayaType, parentMayaType, extraAttrs=None):
    ...
    """
    create a PyNode type for a mya node.
    ...
    if parentPyNodeTypeName is None:
        _logger.raiseLog(_logger.WARNING,
                         'trying to create PyNode for mya type %r, but could'
...
def removePyNode(module, mayaType):
    ...
    if not pyNodeTypeName:
        _logger.raiseLog(_logger.WARNING,
                         'trying to remove PyNode for mya type %r, but could '
...
class VirtualClassManager(object):
    ...
    def register(self, vclass, nameRequired=False, isVirtual='_isVirtual',
        ...
        and mya.cmds. Keep in mind that once your new type is registered, its
...
def isMayaType(mayaType):
    # type: (str) -> bool
    """
    Whether the given type is a currently-defined mya node name
