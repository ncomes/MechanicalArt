...
def mayaIsRunning():
    # type: () -> bool
    """
    Returns True if mya.cmds have  False otherwise.
...
def mayaDocsLocation(version=None):
    ...
    if (not docLocation and (version is None or version == versions.installName())
            and mayaIsRunning()):
        # Return the doc location for the running version of mya
    ...
    # Want the docs for a different version, or mya isn't initialized yet
...
class NodeHierarchyDocParser(HTMLParser):
    ...
    def parse(self):
        ...
        if not os.path.isdir(docloc):
            raise IOError("Cannot find mya documentation. Expected to find it at %s" % docloc)
...
class ParamInfo(object):
    ...
    # featured more prominently in the mya api help docs, and because it's
...
class ApiDocParser(with_metaclass(ABCMeta, object)):
    ...
    def __init__(self, apiModule, version=None, verbose=False, enumClass=tuple,
        ...
        if not os.path.isdir(self.docloc):
            raise IOError("Cannot find mya documentation. Expected to find it at %s" % self.docloc)
        self.strict = strict
...
class HtmlApiDocParser(ApiDocParser):
    ...
    def parseMethodArgs(self, returnType, names, types, typeQualifiers):
        ...
        # have used the pre-2012 format; so we can't just go by mya version...
