...
"""
TEK Node for handling cinematic sequences
"""
...
class CineSequenceComponent(tek.TEKNode):
	VERSION = 1
	...
	def create(seq_data):
		...
		:return: Returns the created TEKNode
		...
		node = (tek.TEKNode.create(tek_parent=None,
											tek_type=CineSequenceComponent.__name__,
		...
		tek_node = CineSequenceComponent(maya_node)
		return tek_node
...
def find_cine_seq_component():
	...
	for network_node in scene_network_nodes:
		try:
			...
			if isinstance(tek_node, (tek.CineSequenceComponent)):
				return tek_node
		except Exception:
			logger.info(f'{network_node} cannot be a TEKNode')
			pass
	return None