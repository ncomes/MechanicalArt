...
def get_python_path_dialog():
    ...
    default_folder = r'C:'
    mca_dir, _ = QFileDialog.getOpenFileName(None,
                                             'Select the python config file.',
                                             default_folder,
                                             "Config (*.config)")
    return os.path.split(mca_dir)[0] if mca_dir else ''
...
def get_ue_project(default_folder=None):
    """
    Opens a file dialog to set the mca main python folder.
    """
    if not default_folder:
        default_folder = r'C:'
    ue_project = QFileDialog.getOpenFileName(None,
                                                'Select an Unreal Project',
                                                default_folder,
                                                "Images (*.uproject)")
    if not ue_project:
        return
    ue_project = ue_project[0]
    file_type = ue_project.split('.')[-1]
    if not file_type == 'uproject':
        return
