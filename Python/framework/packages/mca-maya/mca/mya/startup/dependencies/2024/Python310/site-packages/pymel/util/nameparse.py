...
class MayaShortName(NameParsed):
    ...
    def addPrefix(self, prefix):
        """Add a prefix to the node name. This must produce a valid mya name (no separators allowed)."""
        self.setBaseName(prefix + str(self.getBaseName()))

    def addSuffix(self, suffix):
        """Add a suffix to the node name. This must produce a valid mya name (no separators allowed)."""
...
#    @property
#    def parts(self):
#        """ All parts of that mya short name, that is a possible namespace and node name """
#        return self.sub
...
#                p[0] = MayaNodePath(p[1])
    ...
    def addPrefix(self, prefix):
        """Add a prefix to all nodes in the path. This must produce a valid mya name (no separators allowed)."""
        for node in self.nodes:
            node.setBaseName(prefix + str(node.getBaseName()))

    def addSuffix(self, suffix):
        """Add a suffix to all nodes in the path. This must produce a valid mya name (no separators allowed)."""
        for node in self.nodes:
...
def getBasicPartList(name):
    """
    convenience function for breaking apart a mya object to the appropriate level for pymel name parsing
...
def parse(name):
    """main entry point for parsing a mya node name"""
