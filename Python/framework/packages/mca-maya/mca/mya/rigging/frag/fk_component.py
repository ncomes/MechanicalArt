...
class FKComponent(keyable_component.KeyableComponent):
    ...
    def create(frag_parent,
        ...
        node.connect_nodes(chain_fk_result['flags'], 'fkFlags', 'fragParent')
        node.connect_nodes(chain_fk, 'chainFk', 'fragParent')
        node.connect_nodes(chain_between, 'bindJoints')

        node.addAttr('offsetFlag', at='message')
        if offset_flag:
            node.connect_nodes(new_offset_flag, 'offsetFlag', 'fragParent')
