...
class WorldComponent(keyable_component.KeyableComponent):
    ...
    def create(tek_parent,
        ...
        node.connect_nodes([world_flag, offset_flag, root_flag], 'flags', 'tekParent')
        node.connect_nodes(world_chain, 'worldChain', 'tekParent')
        node.connect_node(world_flag, 'worldFlag', 'tekParent')
        node.connect_node(root_flag, 'rootFlag', 'tekParent')
        node.connect_node(offset_flag, 'offsetFlag', 'tekParent')
    ...
    def get_flags(self):
        return [tek_flag.Flag(x) for x in self.pynode.flags.get()]

    @property
    def world_flag(self):
        return tek_flag.Flag(self.worldFlag.get())

    @property
    def root_flag(self):
        return tek_flag.Flag(self.rootFlag.get())

    @property
    def offset_flag(self):
        return tek_flag.Flag(self.offsetFlag.get())
