...
def create_common_envs(skip_dialog=True):
	...
	os.environ[consts.MCA_COMMON_TOP_ROOT] = common.TOP_ROOT
	if not skip_dialog:
		logger.info(f'{consts.MCA_COMMON_TOP_ROOT} environment variable -path to the base common folder- set to:'
					f'\n{common.TOP_ROOT}')
	
	# Set base Common path env.
	configs_folder = os.path.join(common.COMMON_PATH, 'configs')
	os.environ[consts.MCA_CONFIGS_PATH] = configs_folder
	if not skip_dialog:
