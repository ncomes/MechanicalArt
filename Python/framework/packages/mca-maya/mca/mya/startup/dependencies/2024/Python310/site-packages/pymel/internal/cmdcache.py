...
# exclude them when creating classes from commands. because the mya docs do
...
def getCmdInfo(command, version, python=True):
    ...
    """Since many mya Python commands are builtins we can't get use getargspec on them.
    besides most use keyword args that we need the precise meaning of ( if they can be be used with
    edit or query flags, the shortnames of flags, etc) so we have to parse the mya docs"""
...
def fixCodeExamples(style='mya', force=False):
    """cycle through all examples from the mya docs, replacing mya.cmds with pymel and inserting pymel output.

    NOTE: this can only be run from gui mode
    WARNING: back up your preferences before running

    TODO: auto backup and restore of mya prefs
    """
    import maya.cmds as cmds
    ...
    try:
        for command in sortedCmds:
            ...
            example = example.replace('import mya.cmds as cmds', 'import pymel.core as pm' + DOC_TEST_SKIP, 1)
            example = reg.sub('pm.', example)

            # example = example.replace( 'import mya.cmds as cmds', 'import pymel as pm\npm.newFile(f=1) #fresh scene' )
            ...
            # narrowed down the commands that cause mya to crash to these prefixes
            ...
            try:  # funky things can happen when executing mya code: some
        ...
        for flag in sorted(flags.keys()):
            ...
            if 'query' in modes or testModes == True:
                ...
                try:
                    ...
                    # some flags are only in mel help and not in mya docs, so we don't know their
                ...
                try:
                    ...
                    if not cmds.objExists(obj):
                        ...
                        if objType == 'camera' and flag == 'name':
                            ...
                            # but as long as it's the same mya type, and isn't
...
class CmdCache(cachebase.SubItemCache):
    ...
    def rebuild(self):
        ...
        WARNING: will unload existing plugins, then (re)load all mya-installed
        ...
        loading all the plugins may crash mya, especially if done from a
        non-GUI session
        """
        import maya.cmds as cmds
        ...
        _logger.info("Rebuilding the mya node hierarchy...")
        ...
        # ... sometimes functions get left in mya.cmds... and then trying
