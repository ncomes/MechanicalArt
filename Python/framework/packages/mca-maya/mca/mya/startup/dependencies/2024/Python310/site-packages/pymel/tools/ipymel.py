...
- magic commands
- bookmarking of mya's recent project and files
...
try:
    ...
    print("ipymel can only be setup if the mya package can be imported")
    raise e
...
# don't import pymel here, as this will trigger loading of mya/pymel
# immediately, and things in the userSetup.py won't get properly entered into
# the ipython shell's namespace... we need the startup of mya to happen
# from "within" ipython, ie, when we do:
#   ip.ex("from pymel.core import *")
# from pymel import core

# we also can't even use mya.cmds, because it doesn't work in anything other
...
def api_ls(args, dagOnly, long=False):
    ...
    seem to work very well from a subthread, use mya.api.OpenMaya'''
...
# mya sets a sigint / ctrl-c / KeyboardInterrupt handler that quits mya -
...
# unfortunately, it seems mya overrides the SIGINT hook whenever a plugin is
# loaded...
...
for _mayaproj in optionVar.get('RecentFilesList', []):
    ...
    # setup a handler for ctrl-c / SIGINT / KeyboardInterrupt, so mya / ipymel
    # doesn't quit
