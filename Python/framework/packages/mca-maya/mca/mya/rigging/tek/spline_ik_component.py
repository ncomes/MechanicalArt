...
class SplineIKComponent(keyable_component.KeyableComponent):
    ...
    def create(tek_parent,
               ...
               **kwargs):
        """
        This will create a new complex splineIK rig component for use within the TEK system.
        ...
        node.connect_nodes(spline_ik_results['flag_nodes'], 'flags', 'tekParent')
        ...
        node.connect_node(spline_skin_cluster, 'splineSkinCluster', 'tekParent')
        node.connect_node(ik_solver, 'ikSolver', 'tekParent')
        node.connect_node(mid_flag_grp, 'midFlagGroup', 'tekParent')
        node.connect_node(main_flags_grp, 'mainFlagsGroup', 'tekParent')

        if can_retract:
            node.connect_nodes(remap_nodes, 'remapNodes', 'tekParent')
            node.connect_node(motion_path, 'motionPath', 'tekParent')
            node.connect_nodes(mult_nodes, 'multNodes', 'tekParent')
            node.connect_node(condition, 'condition', 'tekParent')
            node.connect_node(constraint_reverse, 'reverseNodes', 'tekParent')
