...
def ue_root_path():
    path = os.getenv('UE_PROJECT_PATH', None)
    if not path or os.path.exists(path):
        logger.error(f'UE_PROJECT_PATH Not Set!')
        return
    return os.path.dirname(path)


MCA_UNREAL_ROOT = ue_root_path()
...
def get_package_dict():
    ...
    return jsonio.read_json(config_file)
...
class MCAProjectPrefs:
    ...
    def save(self):
        ...
        yamlio.write_yaml(prefs_file, self._data)
