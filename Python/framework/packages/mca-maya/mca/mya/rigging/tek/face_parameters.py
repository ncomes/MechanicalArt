...
class FragFaceParameters(tek_base.TEKNode):
	VERSION = 1.0
	
	@staticmethod
	@ma_decorators.keep_selection_decorator
	@ma_decorators.keep_namespace_decorator
	def create(tek_parent, parameters_list, usage=''):
		"""
		...
		node = (tek_base.TEKNode.create(tek_parent=tek_parent,
											tek_type=FragFaceParameters.__name__,
		...
		for x in range(len(parameters_list)):
			...
			# Add parameter info
			pose_data_dict = tek_node.create_pose_data_dict(parameters_list[x])
	...
	def config_input(self, obj, attribute_name, start_value, end_value, parameter_name):
		...
		Must be connected to a Frag node or be a TEK node.
		...
		if tek_base.is_tek_node(obj):
			tek_parent = tek_base.TEKNode(obj)
		else:
			tek_parent = tek_base.get_tek_parent(obj)
			
		if not tek_parent:
			raise ValueError("object given isn't connected to a TEK node")
		...
		# Call config_input on the tek_parent
		tek_parent.config_input(obj,
									attribute_name,
									start_value,
									end_value,
									parameter_name,
		...
		if unit_conv_node:
			self.connect_nodes(unit_conv_node, 'unitConvNodes', 'tekParent')
		tek_parent.message >> compound_attribute.input
	...
	def get_all_active_poses_and_value(self):
		...
		for x in range(self.pynode.parameters.numElements()):
			...
			if self.is_pose_activated(parameter_attr):
				pose_name = parameter_attr.parameter.get()
				component = tek_base.TEKNode(parameter_attr.input.get())
