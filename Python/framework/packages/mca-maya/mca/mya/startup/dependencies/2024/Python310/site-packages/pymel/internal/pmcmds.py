'''
This module wraps mya.cmds to accept special pymel arguments.
...
If you're having compatibility issues with your custom classes when passing them to mya.cmds,
...
def getCmdName(inFunc):
    ...
    '''Use in place of inFunc.__name__ when inFunc could be a mya.cmds cmd
    ...
    if cmdName == 'stubFunc':
        ...
        if (isinstance(sourceFile, basestring) and
                os.path.join('mya', 'app', 'commands') in sourceFile):
            # Here's where it gets tricky... this is a fairly big hack, highly
            # dependent on the exact implementation of mya.app.commands.stubFunc...
...
def addWrappedCmd(cmdname, cmd=None):
    ...
    def wrappedCmd(*args, **kwargs):
        # we must get the cmd each time, because mya delays loading of functions until they are needed.
        ...
        # when editing, some of mya.cmds functions return empty strings and
    ...
    # so that we can identify that this is a wrapped mya command
