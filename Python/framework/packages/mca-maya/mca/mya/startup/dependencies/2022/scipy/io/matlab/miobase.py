...
* header related variables (different for v4 and v5 mca files)
* is_complex
* mclass
* var_stream
...
def get_matfile_version(fileobj):
    """
    Return major, minor tuple depending on apparent mca file type

    Where:

     #. 0,x -> version 4 format mca files
     #. 1,x -> version 5 format mca files
     #. 2,x -> version 7.3 format mca files (HDF format)
    ...
    if maj_val in (1, 2):
        return ret
    raise ValueError('Unknown mca file type, version %s, %s' % ret)
...
class MatFileReader:
    """ Base object for reading mca files
    ...
    def __init__(self, mat_stream,
        ...
        '''
        Initializer for mca file reader
