    ...
    if destination:
        ...
        for disconnectDir in disconnectionDirs:
            ...
            connections = cmds.listConnections(source,
                                               ...
                                               connections=True,
                                               plugs=True)
            # stupid mya.cmds returns None instead of []...
...
def getAttr(attr, default=None, **kwargs):
    """
    Maya Bug Fix:
      - mya pointlessly returned vector results as a tuple wrapped in a list ( ex.  '[(1,2,3)]' ). This command unpacks the vector for you.
    ...
    Modifications:
      ...
      - when getting a multi-attr, mya would raise an error, but pymel will return a list of values for the multi-attr
...
def setAttr(attr, *args, **kwargs):
    ...
    Modifications:
      ...
      - Added 'force' kwarg, which causes the attribute to be added if it does not exist.
            ...
            - currently supported python-to-mya mappings:

                ============ ===========
                python type  mya type
                ============ ===========
                float        double
                ------------ -----------
                int          long
...
def addAttr(*args, **kwargs):
    ...
    if kwargs.get('q', kwargs.get('query', False)):
        ...
        if queriedArg in ('dt', 'dataType'):
            # If the attr is not a dynamic attribute, mya.cmds prints:
            #    Error: '...' is not a dynamic attribute of node '...'.
            # ...but does NOT raise an exception
            # Because it will be more consistent with mya.cmds, and because
            # attributeType already behaves like this, we will do the same -
            # allow mya.cmds to print it's error message, and return None, but
...
def ls(*args, **kwargs):
    ...
    if regexArgs:
        ...
        # was a mya bug that would sometimes make nodes have no long name:
...
#            # mya only goes one deep, and only checks for lists or tuples
#            for subarg in arg:
...
def nodeType(node, **kwargs):
    # type: (Any, **Any) -> str
    """
    Note: this will return the dg node type for an object, like mya.cmds.nodeType,
...
def parent(*args, **kwargs):
    ...
    if nodes and not removeObj:
        ...
        # if you try to parent to the current parent, mya errors...
...
#             luma.logger.debug("deregistered node added callback")
    ...
    def getNodes(self, returnType='PyNode'):
        # type: (str) -> None
        """
        Return a list of mya objects as strings.
...
class PyNode(_util.ProxyUnicode):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if args:
            ...
            if len(args) > 1:
                ...
                elif hasattr(argObj, '__module__') and argObj.__module__.startswith('mya.OpenMaya'):
                    pass
    ...
    def __apimfn__(self):
        """Get a ``mya.OpenMaya*.MFn*`` instance
        """
        try:
...
class Attribute(with_metaclass(_factories.MetaMayaTypeRegistry, PyNode)):
    ...
    For the most part, the names of the class equivalents to the mya.cmds functions follow a fairly simple pattern:
    ...
    The shorthand method is the most visually appealing and readable -- you simply access the mya attribute as a normal python attribute --
...
#        except: return None
    ...
    def numElements(self):
        ...
        that you use `Attribute.numElements` instead.  This is a mya bug, *not* a pymel bug.
...
#        return datatypes.Vector( cmds.xform( self, **kwargs ) )
    ...
    def setLocked(self, locked, checkReference=CHECK_ATTR_BEFORE_LOCK):
        ...
        checkReference : `bool`
            ...
            By default pymel and the mya api do not check if the node is referenced before
...
class DimensionedComponent(Component):
    ...
    def _makeIndexedComponentHandle(self, indices):
        """
        Returns an MObjectHandle that points to a mya component object with
        the given indices.
        """
        selList = _api.MSelectionList()
...
class DiscreteComponent(DimensionedComponent):
    ...
    def _sliceToIndices(self, sliceObj, partialIndex=None):
        ...
        # convert 'mya slices' to 'python slices'...
        # ie, in mya, someObj.vtx[2:3] would mean:
...
#                for rawIndex in range(start, stop, step)]
    ...
    def _dimensionIter(self):
        ...
        # - only it's stored in mya's private memory space - we AVOID
    ...
    def _flatIter(self):
        ...
        # itself, and is stored in mya's memory, but the idea is the same...
...
class Component1D(DiscreteComponent):
    ...
    def _sequenceToComponentSlice(array):
        """given an array, convert to a mya-formatted slice"""
...
class SubdEdge(Component1D64):
    ...
    # ...mya crashes. as a hack to to help avoid crashing, define the complete
...
#        return max + 1
    ...
    # To see what I mean, uncomment this and try it in mya:
#from pymel.core import *
#import sys
#import platform
#
