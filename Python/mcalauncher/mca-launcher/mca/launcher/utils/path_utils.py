...
def get_standalone_path():
    standalone_path = os.path.normpath(os.path.join(get_mca_python_root_path(), 'Python', 'standalone'))
    if not os.path.exists(standalone_path):
        logger.warning(f'Could not find the standalone folders. {standalone_path}')
        return

    paths = os.path.normpath(standalone_path)
    return paths
...
def get_maya_root():
    package_path = get_package_path()
    maya_root = os.path.join(package_path, 'mca-maya')
    return os.path.normpath(maya_root)


def get_maya_path():
    package_path = get_maya_root()
    maya_path = os.path.join(package_path, 'mca', 'mya')
    return os.path.normpath(maya_path)


def get_maya_dep_path():
    package_path = get_maya_path()
    maya_path = os.path.join(package_path, 'startup', 'dependencies')
    return os.path.normpath(maya_path)
...
def get_launcher_path():
    ...
    package_path = os.path.join(get_standalone_path(), 'mcalauncher')
    if not package_path:
        return
...
def get_dependencies_path():
    ...
    if not common_path:
        return
    dependency_path = os.path.join(common_path, 'startup', 'dependencies')
