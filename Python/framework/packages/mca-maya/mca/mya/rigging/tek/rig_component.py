...
class RigComponent(tek_base.TEKNode):
    """
    All TEK rig components should be of this type.
    This represents the base node for all potential TEK components that could be used with a rig.
    ...
    def create(tek_parent, tek_type, version, side, region, align_component=None):
        ...
        node = tek_base.TEKNode.create(tek_parent, tek_type, version)
        ...
        if tek_parent.get_type() == "TEKRig":
            nt_grp.setParent(tek_parent.doNotTouch.get())

        node.connect_node(nt_grp, 'noTouch', 'tekParent')
        # Add rig attributes
        node.side = side
        node.region = region

        # This is a list of child components attached to this TEK node.
    ...
    def _return_tek_attachments(self, attached_tek_list):
        components = [tek_base.TEKNode(x) for x in attached_tek_list]
        return components

    @property
    def attached_children(self):
        return self._return_tek_attachments(self.pynode.getAttr('childAttachments'))

    @property
    def attached_tek_parents(self):
        return self._return_tek_attachments(self.pynode.getAttr('pointParentAttachments'))

    @property
    def orient_tek_parents(self):
        return self._return_tek_attachments(self.pynode.getAttr('orientwintParentAttachments'))
    ...
    def serialize_component(self):
        """
        Converts a TEK component into a dictionary of build instructions for regeneration later.
            ...
            for attr_name in attr_list:
                ...
                if constraint_node and constraint_node not in axis_constraint_list:
                    ...
                    if not primary_constraint_dict[constraint_type]:
                        if tek_flag.is_flag_node(target_node):
                            # we have a flag
                            constraint_data_dict = {}
                            flag_node = tek_flag.Flag(target_node)
                            rig_node = tek_base.TEKNode(flag_node.tekParent.get())
        ...
        if hasattr(self, 'get_flags'):
            ...
            for index, wrapped_flag_node in enumerate(flag_node_list):
                if wrapped_flag_node.node.hasAttr('sourceMultiConstraint'):
                    multiconstraint_node = tek_base.TEKNode(wrapped_flag_node.sourceMultiConstraint.get())
