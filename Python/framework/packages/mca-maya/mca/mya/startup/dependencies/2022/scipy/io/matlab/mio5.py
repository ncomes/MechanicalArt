...
'''
=================================
 Note on functions and mca files
=================================
...
``mxOPAQUE_CLASS`` by looking at example mca files.
...
It seems that whenever the mca file contains a ``mxOPAQUE_CLASS``
instance, there is also an un-named matrix (name == '') at the end of
the mca file. I'll call this the ``__function_workspace__`` matrix.

When I saved two anonymous functions in a mca file, or appended another
anonymous function to the mca file, there was still only one
``__function_workspace__`` un-named matrix at the end, but larger than
that for a mca file with a single anonymous function, suggesting that
...
the format of a mini .mca file, without the first 124 bytes of the file
...
entries, as in a standard mca file. The ``miMATRIX`` entries appear to
be series of un-named (name == '') matrices, and may also contain arrays
of this same mini-mca format.

I guess that:

* saving an anonymous function back to a mca file will need the
  associated ``__function_workspace__`` matrix saved as well for the
  anonymous function to work correctly.
* appending to a mca file that has a ``__function_workspace__`` would
  ...
  merging the relevant workspaces, and saving at the end of the mca
  file.

The mca files I was playing with are in ``tests/data``:
...
def _simplify_cells(d):
    """Convert mca objects in dict to nested dicts."""
    for key in d:
...
def varmats_from_mat(file_obj):
    """ Pull variables out of mca 5 file as a sequence of mca file objects

    This can be useful with a difficult mca file, containing unreadable
    ...
    ----------
    file_obj : file-like
        file object containing mca file

    Returns
    -------
    named_mats : list
        list contains tuples of (name, BytesIO) where BytesIO is a file-like
        object containing mca file contents as for a single variable. The
        BytesIO contains a string with the original header and a single var. If
        ``var_file_obj`` is an individual BytesIO instance, then save as a mca
        file with something like ``open('test.mca',
...
class VarWriter5:
    ...
    def write_top(self, arr, name, is_global):
        """ Write variable at top level of mca file
