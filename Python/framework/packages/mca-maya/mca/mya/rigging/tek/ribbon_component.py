...
class IKFKRibbonComponent(keyable_component.KeyableComponent):
    ...
    def create(tek_parent,
        ...
        node.connect_node(ik, 'ik_handle', 'tekParent')
        ...
        node.connect_node(invert_node, 'invert_node', 'tekParent')
        ...
        node.connect_nodes(detail_flags, 'detailFlags', 'tekParent')
        node.connect_node(skin_cluster_node, 'ribbonSkinCluster', 'tekParent')
...
class SplineIKRibbonComponent(keyable_component.KeyableComponent):
    ...
    def create(tek_parent,
        ...
        for joint_node, identifier in zip(ribbon_simp_list, ['start', 'mid', 'end']):
            primary_flag = tek_flag.Flag.create(joint_node, label=f'{side}_{region}_{identifier}', scale=scale)
        ...
        node.connect_nodes(chain_between, 'bindJoints')
        node.connect_nodes(detail_flags, 'detailFlags', 'tekParent')
        node.connect_nodes(primary_flags, 'flags', 'tekParent')
        node.connect_node(ribbon_node, 'ribbon', 'tekParent')
        node.connect_node(spline_ik_handle, 'ikHandle', 'tekParent')
        node.connect_node(info_node, 'infoNode', 'tekParent')
        node.connect_nodes(multi_nodes, 'multiNodes', 'tekParent')
        node.connect_nodes([ribbon_skin_cluster, curve_skin_cluster], 'ribbonSkinCluster', 'tekParent')
        node.connect_node(ribbon_tweak, 'ribbonTweak', 'tekParent')
        node.connect_nodes(make_plane_node, 'makePlaneNode', 'tekParent')
        node.connect_node(spline_solver, 'ikSolver', 'tekParent')
        node.connect_nodes(unit_conversion_node, 'unitConvNodes', 'tekParent')
