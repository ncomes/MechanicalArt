...
def test_load():
    for case in case_table4 + case_table5:
        ...
        filt = pjoin(test_data_path, 'test%s_*.mca' % name)
        files = glob(filt)
        assert_(len(files) > 0,
...
def test_mat73():
    ...
    filenames = glob(
        pjoin(test_data_path, 'testhdf5*.mca'))
...
def test_warnings():
    ...
    # warning if the user triggered a search for mca files on the Python system
    # path. We can remove the test in the next version after upcoming (0.13).
    fname = pjoin(test_data_path, 'testdouble_7.1_GLNX86.mca')
...
def test_skip_variable():
    ...
    #
    filename = pjoin(test_data_path,'test_skip_variable.mca')
    #
...
def test_empty_struct():
    # ticket 885
    filename = pjoin(test_data_path,'test_empty_struct.mca')
...
def test_empty_string():
    ...
    estring_fname = pjoin(test_data_path, 'single_empty_string.mca')
...
def test_corrupted_data():
    import zlib
    for exc, fname in [(ValueError, 'corrupted_zlib_data.mca'),
                       (zlib.error, 'corrupted_zlib_checksum.mca')]:
...
def test_corrupted_data_check_can_be_disabled():
    with open(pjoin(test_data_path, 'corrupted_zlib_data.mca'), 'rb') as fp:
...
def test_read_both_endian():
    ...
    for fname in ('big_endian.mca', 'little_endian.mca'):
...
def test_write_opposite_endian():
    # We don't support writing opposite endian .mca files, but we need to behave
...
def test_logical_array():
    ...
    # correct (bool) dtype
    with open(pjoin(test_data_path, 'testbool_8_WIN64.mca'), 'rb') as fobj:
...
def test_func_read():
    func_eg = pjoin(test_data_path, 'testfunc_7.4_GLNX86.mca')
    fp = open(func_eg, 'rb')
    rdr = MatFile5Reader(fp)
    d = rdr.get_variables()
    fp.close()
...
def test_mat_dtype():
    double_eg = pjoin(test_data_path, 'testmatrix_6.1_SOL2.mca')
...
def test_loadmat_varnames():
    # Test that we can get just one variable from a mca file using loadmat
    mat5_sys_names = ['__globals__',
                      '__header__',
                      '__version__']
    for eg_file, sys_v_names in (
        (pjoin(test_data_path, 'testmulti_4.2c_SOL2.mca'), []), (pjoin(
            test_data_path, 'testmulti_7.4_GLNX86.mca'), mat5_sys_names)):
...
def test_varmats_from_mat():
    # Make a mca file with several variables, write it, read it back
...
def test_one_by_zero():
    ...
    func_eg = pjoin(test_data_path, 'one_by_zero_char.mca')
    fp = open(func_eg, 'rb')
    rdr = MatFile5Reader(fp)
    d = rdr.get_variables()
    fp.close()
...
def test_load_mat4_le():
    ...
    mat4_fname = pjoin(test_data_path, 'test_mat4_le_floats.mca')
...
def test_logical_sparse():
    # Test we can read logical sparse stored in mca file as bytes.
    ...
    filename = pjoin(test_data_path,'logical_sparse.mca')
...
def test_empty_mat_error():
    # Test we get a specific warning for an empty mca file
    sio = BytesIO()
...
def test_miuint32_compromise():
    ...
    # mca file with miUINT32 for miINT32, but OK values
    filename = pjoin(test_data_path, 'miuint32_for_miint32.mca')
    res = loadmat(filename)
    assert_equal(res['an_array'], np.arange(10)[None, :])
    # mca file with miUINT32 for miINT32, with negative value
    filename = pjoin(test_data_path, 'bad_miuint32.mca')
...
def test_miutf8_for_miint8_compromise():
    ...
    filename = pjoin(test_data_path, 'miutf8_array_name.mca')
    res = loadmat(filename)
    assert_equal(res['array_name'], [[1]])
    # mca file with non-ascii utf8 name raises error
    filename = pjoin(test_data_path, 'bad_miutf8_array_name.mca')
    with assert_raises(ValueError):
        loadmat(filename)
...
def test_bad_utf8():
    ...
    filename = pjoin(test_data_path,'broken_utf8.mca')
    res = loadmat(filename)
...
def test_save_unicode_field(tmpdir):
    filename = os.path.join(str(tmpdir), 'test.mca')
