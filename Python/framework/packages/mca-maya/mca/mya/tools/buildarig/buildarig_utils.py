...
def serialize_rig(frag_rig, version=None):
    ...
    if not version:
        version = int(frag_rig.rigVersion.get() + 1)
    frag_rig.rigVersion.set(version)
    rig_build_list = [{'version': version}]

    for frag_component in frag_rig.get_frag_children():
        if isinstance(frag_component, frag.TwistFixUpComponent) or not isinstance(frag_component, (frag.RigComponent, frag.MultiConstraint)):
            continue
...
def save_serialized_rig_cmd():
    ...
    save_serialized_rig(frag_rig, found_path)
...
def save_serialized_rig(frag_rig, rig_path):
    ...
    serialized_build_list = serialize_rig(frag_rig)
...
def load_serialized_rig(frag_rig, rig_path):
    ...
    build_serialized_rig(frag_rig, serialized_build_list)
