...
that mya is initialized in standalone mode.
"""
...
# There are FOUR different ways mya might be started, all of which are
...
def mayaStartupHasRun():
    """
    Returns True if mya.app.startup has already finished, False otherwise.
    """
    return 'mya.app.startup.gui' in sys.modules or 'mya.app.startup.batch' in sys.modules


def mayaStartupHasStarted():
    """
    Returns True if mya.app.startup has begun running, False otherwise.

    It's possible that mya.app.startup is in the process of running (ie,
    in mya.app.startup.basic, calling executeUserSetup) - unlike mayaStartup,
...
def setupFormatting():
    ...
    maya.utils.formatGuiResult.__module__ = 'mya.utils'

# def loadDynamicLibs():
#    """
#    due to a bug in mya.app.commands many functions do not return any value the first time they are run,
...
#
#    this function can only be run after mya.standalone is initialized
#    """
#
...
#
#    import mya.cmds
#    assert hasattr( mya.cmds, 'dynamicLoad'), "mya.standalone must be initialized before running this function"
#    file = open( commandListPath, 'r' )
#    libraries = set( [ line.split()[1] for line in file] )
#    for library in libraries:
#        try:
#            mya.cmds.dynamicLoad(library)
#        except RuntimeError:
#            _logger.debug("Error dynamically loading mya library: %s" % library)

# Will test initialize mya standalone if necessary (like if scripts are run from an exernal interpeter)
...
def _mayaInit(forversion=None):
    ...
    Part of the complexity of initializing mya in standalone mode is that mya does not populate os.environ when
    parsing Maya.env.  If we initialize normally, the env's are available via mya (via the shell), but not in python
    via os.environ.

    Note: the following example assumes that MAYA_SCRIPT_PATH is not set in your shell environment prior to launching
    python or mayapy.

    >>> import mya.standalone            #doctest: +SKIP
    >>> mya.standalone.initialize()      #doctest: +SKIP
    >>> import mya.mel as mm             #doctest: +SKIP
    ...
    The solution lies in `refreshEnviron`, which copies the environment from the shell to os.environ after mya.standalone
    initializes.

    Returns
    -------
    bool
        returns True if mya.cmds required initializing ( in other words, we are in a standalone python interpreter )
    ...
    if aboutExists and mayaStartupHasStarted():
        ...
        _logger.debug("startup.mayaInit: mya already started - exiting")
        isInitializing = False
        return False

    _logger.debug("startup.mayaInit: running")

    # for use with pymel compatible mya package
    oldSkipSetup = os.environ.get('MAYA_SKIP_USERSETUP_PY')
    os.environ['MAYA_SKIP_USERSETUP_PY'] = 'on'
    try:
        if not aboutExists and 'mya.standalone' not in sys.modules:
            try:
                _logger.debug("startup.mayaInit: running standalone.initialize")
                import maya.standalone  # @UnresolvedImport
                maya.standalone.initialize(name="python")
            except ImportError as e:
                raise ImportError("%s: pymel was unable to intialize mya.standalone" % e)

        try:
            from maya.cmds import about
        except Exception:
            _logger.error("mya.standalone was successfully initialized, but pymel failed to import mya.cmds (or it was not populated)")
            raise
    finally:
    ...
    if not mayaStartupHasRun():
        _logger.debug("running mya.app.startup")
        # If we're in 'mya -prompt' mode, and a plugin loads pymel, then we
        # can have a state where mya.standalone has been initialized, but
    ...
    # return True, meaning we had to initialize mya standalone
    isInitializing = True
    return True
...
def initMEL():
    ...
    startup = [
        ...
        'initialStartup.mel',
        #$HOME/Documents/mya/projects/default/workspace.mel
        'initialPlugins.mel',
        #'initialGUI.mel', #GUI
    ...
    if pymel_options.get('skip_initial_plugins', False):
        # initialPlugins.mel is not sourced when running mya -batch, but has been included
...
def finalize():
    ...
    if isInitializing:
        ...
        assert 'mya.app.startup.basic' in sys.modules, \
            "something is very wrong. mya.app.startup.basic should be imported by now"
...
def fixMayapySegFault():
    # first, install a mya exit callback that will let us know if uninitialize
    # has already been called...
    def mayaExitCallback(data):
        global _mayaUninitialized
        _mayaUninitialized = True
        import maya.cmds
        # Turn off undo, because mya seems to have a bug where uninitialize()
        # will crash if undo is on, and certain custom conditions have been
        # created - ie, from the command line:

        # mayapy -c 'import mya.standalone
        # mya.standalone.initialize()
        # import mya.cmds as cmds
        # import mya.OpenMaya as om
        ...
        # mya.standalone.uninitialize()'; echo $?
        # # Seg fault - 139
...
def fixMayapy2011SegFault():
    ...
    if platform.system() == 'Linux':
        if om.MGlobal.mayaState() == om.MGlobal.kLibraryApp:  # mayapy only
            ...
            # In linux mya 2011, once mya has been initialized, if you try
        ...
        if mayaEncode != pyEncode:             # s'il faut redefinir l'encoding
            ...
            if not about(b=True):              # si pas en batch, donc en mode UI, redefinir stdout et stderr avec encoding Maya
                ...
                try:
                    ...
                    _logger.debug("Unable to import mya.app.baseUI")
