...
from mca.mya.rigging.frag import frag_base, face_parameters, frag_rig
...
def get_face_edit_node(rig_node, skip_dialog=False):
	...
	frag_rig_node = frag_rig.get_frag_rig(rig_node)
	if not frag_rig_node:
		if not skip_dialog:
			logger.warning('The node given is not connected to a rig or there is not FRAGRig node in the scene.')
		return
	edit_node = frag_rig_node.get_frag_children(of_type=FaceEditComponent)
	if not edit_node:
		if not skip_dialog:
...
class FaceEditComponent(frag_base.FRAGNode):
	VERSION = 1.0
	...
	def create(frag_parent):
		...
		:param frag_parent: FRAG Skeletal Mesh FRAGNode
		...
		node = frag_base.FRAGNode.create(frag_parent=frag_parent,
											frag_type=FaceEditComponent.__name__,
	...
	def set_flags_info(self, main_pose=None):
		...
		frag_parent = self.get_frag_parent()
		parameter_node = frag_parent.get_frag_children(of_type=face_parameters.FragFaceParameters)
	...
	def set_flag_block_value(self, pose_name):
		...
		component = frag_base.FRAGNode(component)
	...
	def get_flag_component_from_block(self, flag_name):
		...
		if not flag_block:
			return
		return frag_base.FRAGNode(flag_block.activeComponent.get())
...
class FaceFlagsData:
	...
	def create(cls, pose_list, parameter_node):
		...
		for pose in pose_list:
			...
			component = frag_base.FRAGNode(parameter_node.attr(pose).input.get())
...
class FlagAttrData(object):
	def __init__(self, rig_flag, flag_attr, value, pose, flag_attr_max, parameter_value, component):
		self.rig_flag = frag_flag.Flag(rig_flag)
